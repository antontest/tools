...
function EditCell(element, editType){  
    ...
    switch(editType){  
        ...
        case "DropDownList":  
            ...
            break;  
        case "Date":
            CreateDateSelect(element, element.innerHTML);
            break;
        default:  
            break;  
    }  
...
//为单元格创建可编辑输入框  
function CreateDateSelect(element, value){  
    var orivalue = value;
    //检查编辑状态，如果已经是编辑状态，跳过  
    ...
    if(editState != "true"){  
        ...
        element.setAttribute("ChangeState", 1);
        ...
        */
        textBox.oncontextmenu = function () {
            MyCalendar.SetDate(this);
            return false;
        }
        textBox.ondblclick = function (){  
            ...
            if (this.value != orivalue) {
                ...
                if (XlsChangeDataObj) {
                    var rowcol = element.parentNode.rowIndex + "," + element.cellIndex + ",";
                    var start_pos = XlsChangeDataObj.value.indexOf(rowcol);
                    if (start_pos != -1) {
                        for (var i = start_pos; i < XlsChangeDataObj.value.length; i++) {
                            if (XlsChangeDataObj.value[i] == ';') {
                                break;
                            }
                        }
                        var replace_str = XlsChangeDataObj.value.substring(start_pos, i+1);
                        var row=element.parentNode.rowIndex;
                        var col=element.cellIndex;
                        var reg = eval("/"+replace_str+"/");
                        var value=XlsChangeDataObj.value.replace(reg, row+","+col+","+this.value+";");
                        XlsChangeDataObj.value=value;
                    } else {
                        XlsChangeDataObj.value+=element.parentNode.rowIndex + "," + element.cellIndex + "," + this.value + ";";
                    }
                }
            }
...
function GetTableData(table){  
    ...
    var dataObj = document.getElementById("xls_change_data");
    if (dataObj) {
        alert(dataObj.value);
    }
    return tableData;  
